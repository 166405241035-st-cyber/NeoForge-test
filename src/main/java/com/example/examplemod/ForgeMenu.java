...
public class ForgeMenu extends AbstractContainerMenu {
    ...
    /** Client-side menu. Fuel is synchronized by DataSlot. */
    ...
    /** Server-side menu with a fuel value belonging to the opened Forge block. */
    public ForgeMenu(int containerId, Inventory playerInventory, Container forgeInventory, int initialFuel, IntConsumer fuelSaver) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) addSlot(new Slot(playerInventory, col + row * 9 + 9, 8 + col * 18, 126 + row * 18));
        }
    ...
    /** Converts coal/charcoal in the fuel slot into stored fuel, 8 units per item. */
    ...
    public boolean hasValidRecipe() {
        ...
        if (blueprint == ForgingBlueprintType.ROD) return false;
